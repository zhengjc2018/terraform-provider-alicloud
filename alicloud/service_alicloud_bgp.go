...
func (s *BgpService) DescribeBgpNetwork(id string) (v vpc.BgpNetwork, err error) {
	...
	if err != nil {
		if IsExceptedErrors(err, []string{BgpNotFound, BgpInternalError}) {
...
func (s *BgpService) WaitForBgpNetwork(id string, status Status, timeout int) error {
	...
	for {
		...
		if object.RouterId == parts[0] && object.DstCidrBlock == parts[1] && string(status) != string(Deleted) {
			return nil
		}
...
func (s *BgpService) WaitForBgpPeer(id string, status Status, timeout int) error {
	...
	for {
		...
		//	return nil
		//}
		if object.BgpPeerId == id && string(status) != string(Deleted){
			return nil
		}
