...
func resourceAliyunBgpPeerCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if v, ok := d.GetOk("enable_bfd"); ok && v.(string) != "" {
		request.EnableBfd = requests.NewBoolean(v.(bool))
	}
	...
	id := response.BgpPeerId
	d.SetId(id)

	if err := bgpNetworkService.WaitForBgpPeer(d.Id(), Active, 2*DefaultTimeoutMedium); err != nil {
...
func resourceAliyunBgpPeerDelete(d *schema.ResourceData, meta interface{}) error {
	...
	err := resource.Retry(5*time.Minute, func() *resource.RetryError {
		...
		if err != nil {
			if IsExceptedErrors(err, []string{BgpInvalidStatus, BgpPeerDependencyViolation}) {
				wait()
